...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "ru": {
        ...
        "btn.invite_rewards": "📊 Бонусы приглашений",
        ...
        "group_invite.link": "👥 Скопируйте текст ниже и отправьте друзьям:\n<code>{share_text}</code>\n\nБонус приглашения становится доступен после чек-ина нового пользователя и начисляется после периода проверки. Награда: {reward}.",
        ...
        "group_invite.pending_settlement": "👥 Бонус приглашения ожидает проверки и будет начислен позже: +{points} балл(ов).",
        "group_invite.risk_blocked": "👥 Бонус приглашения отправлен на дополнительную проверку риска: +{points} балл(ов).",
        "group_invite.rejected": "👥 Бонус приглашения не будет начислен после проверки: +{points} балл(ов).",
        "group_invite.rewards.title": "📊 <b>Бонусы приглашений</b>",
        "group_invite.rewards.summary": "Всего: {total} | ожидание: {pending} | готово: {qualified} | начислено: {rewarded} | риск: {risk_blocked} | отклонено: {rejected}",
        "group_invite.rewards.empty": "Пока нет приглашений с бонусами.",
        "group_invite.rewards.item": "• Пользователь {invited}: {status}, +{points}, расчёт: {settlement_at}",
        "group_invite.rewards.reason": "Причина: {reason}",
        "group_invite.rewards.has_more": "Показаны последние записи. Более ранние записи доступны оператору.",
        "group_invite.rewards.page": "Стр. {page}",
        "group_invite.rewards.appeal": "Оспорить {invited}",
        "group_invite.rewards.appeal_created": "Апелляция #{appeal_id} отправлена на проверку.",
        "group_invite.rewards.appeal_existing": "Апелляция #{appeal_id} уже находится на проверке.",
        "group_invite.rewards.appeal_unavailable": "Эту запись нельзя оспорить.",
        "group_invite.status.pending": "ожидает чек-ина",
        "group_invite.status.qualified": "ожидает расчёта",
        "group_invite.status.rewarded": "начислено",
        "group_invite.status.risk_blocked": "проверка риска",
        "group_invite.status.rejected": "отклонено",
    ...
    "en": {
        ...
        "btn.invite_rewards": "📊 Invite rewards",
        ...
        "group_invite.link": "👥 Copy the text below and send it to friends:\n<code>{share_text}</code>\n\nInvite bonus becomes eligible after the new user's check-in and is credited after the review window. Reward: {reward}.",
        ...
        "group_invite.pending_settlement": "👥 Invite bonus is pending review and will be credited later: +{points} point(s).",
        "group_invite.risk_blocked": "👥 Invite bonus needs additional risk review: +{points} point(s).",
        "group_invite.rejected": "👥 Invite bonus was rejected after review: +{points} point(s).",
        "group_invite.rewards.title": "📊 <b>Invite rewards</b>",
        "group_invite.rewards.summary": "Total: {total} | pending: {pending} | qualified: {qualified} | credited: {rewarded} | risk: {risk_blocked} | rejected: {rejected}",
        "group_invite.rewards.empty": "No invite rewards yet.",
        "group_invite.rewards.item": "• User {invited}: {status}, +{points}, settlement: {settlement_at}",
        "group_invite.rewards.reason": "Reason: {reason}",
        "group_invite.rewards.has_more": "Showing the latest records. Older records are available to operators.",
        "group_invite.rewards.page": "Page {page}",
        "group_invite.rewards.appeal": "Appeal {invited}",
        "group_invite.rewards.appeal_created": "Appeal #{appeal_id} was submitted for review.",
        "group_invite.rewards.appeal_existing": "Appeal #{appeal_id} is already under review.",
        "group_invite.rewards.appeal_unavailable": "This reward cannot be appealed.",
        "group_invite.status.pending": "waiting for check-in",
        "group_invite.status.qualified": "pending settlement",
        "group_invite.status.rewarded": "credited",
        "group_invite.status.risk_blocked": "risk review",
        "group_invite.status.rejected": "rejected",
...
ZH_OVERRIDES = {
    ...
    "group_invite.link": "👥 复制下面这段发给好友：\n<code>{share_text}</code>\n\n新用户通过链接进群并签到后，邀请奖励进入待结算，审核期结束后发放。奖励：{reward}。",
    ...
    "group_invite.pending_settlement": "👥 邀请奖励已进入待结算，审核期结束后发放：+{points} 积分。",
    "group_invite.risk_blocked": "👥 邀请奖励进入风险复核：+{points} 积分。",
    "group_invite.rejected": "👥 邀请奖励审核未通过，不会发放：+{points} 积分。",
    "group_invite.rewards.title": "📊 <b>邀请奖励</b>",
    "group_invite.rewards.summary": "总数：{total} | 待签到：{pending} | 待结算：{qualified} | 已发放：{rewarded} | 风险：{risk_blocked} | 拒绝：{rejected}",
    "group_invite.rewards.empty": "暂无邀请奖励记录。",
    "group_invite.rewards.item": "• 用户 {invited}：{status}，+{points}，结算：{settlement_at}",
    "group_invite.rewards.reason": "原因：{reason}",
    "group_invite.rewards.has_more": "当前只显示最近记录，更早记录可由运营查询。",
    "group_invite.rewards.page": "第 {page} 页",
    "group_invite.rewards.appeal": "申诉 {invited}",
    "group_invite.rewards.appeal_created": "申诉 #{appeal_id} 已提交复核。",
    "group_invite.rewards.appeal_existing": "申诉 #{appeal_id} 已在复核中。",
    "group_invite.rewards.appeal_unavailable": "这条奖励记录不能申诉。",
    "group_invite.status.pending": "等待签到",
    "group_invite.status.qualified": "待结算",
    "group_invite.status.rewarded": "已发放",
    "group_invite.status.risk_blocked": "风险复核",
    "group_invite.status.rejected": "已拒绝",
...
TRANSLATIONS["en"].update({
    "btn.discover_channels": "🔎 Discover channels",
    "btn.model_lab": "🧪 Interface tests",
    "btn.contribution_tasks": "🤝 Contribution tasks",
    "platform.placeholder.channels": "🔎 Channel discovery is in staged rollout. Use the Mini App/API once enabled by an operator.",
    "platform.placeholder.model_lab": "🧪 Interface testing is in staged rollout. Do not send API keys in chat.",
    "platform.placeholder.contribute": "🤝 Contribution tasks are in staged rollout. Rewards will use audited point ledger entries.",
})
TRANSLATIONS["ru"].update({
    "btn.discover_channels": "🔎 Поиск каналов",
    "btn.model_lab": "🧪 Проверка API",
    "btn.contribution_tasks": "🤝 Задачи вклада",
    "platform.placeholder.channels": "🔎 Поиск каналов включается поэтапно. Используйте Mini App/API после включения оператором.",
    "platform.placeholder.model_lab": "🧪 Проверка API включается поэтапно. Не отправляйте API-ключи в чат.",
    "platform.placeholder.contribute": "🤝 Задачи вклада включаются поэтапно. Награды будут идти через аудитируемый журнал баллов.",
})
TRANSLATIONS["zh"].update({
    "btn.discover_channels": "🔎 发现频道",
    "btn.model_lab": "🧪 接口检测",
    "btn.contribution_tasks": "🤝 贡献任务",
    "platform.placeholder.channels": "🔎 频道发现正在灰度开放。运营开启后请通过 Mini App/API 使用。",
    "platform.placeholder.model_lab": "🧪 接口检测正在灰度开放。请不要在聊天里发送 API Key。",
    "platform.placeholder.contribute": "🤝 贡献任务正在灰度开放。奖励将通过可审计的积分账本发放。",
})
