...
- SSRF: mitigated by `url_safety.py` plus the isolated Worker's runtime DNS and redirect revalidation. Source-controlled isolated-runner templates now exist, but production drain should stay disabled until the root-owned runner, sudoers file, and `tgsellbot-worker` smoke test are installed and approved.
