...
- Model Lab orchestration tables, job status reads, private report reads, user-scoped report lists, owner-controlled visibility changes, public report lists, token-gated unlisted report reads, recent run history, an operator batch drain command, systemd drain templates, sample retention pruning, and dashboard readouts for latest drain/retention outcomes now exist. Mini App Model Lab job/report lists now expose pagination controls with visible range/total state, while report detail separates score badges, job/report timeline, worker run history, visibility meaning, limitation copy, redacted evidence summary, and public/unlisted share controls instead of dumping raw JSON. Sharing supports copy, system share, and Telegram share fallbacks while private reports omit public links. A read-only drain-readiness gate now combines isolated runner, server-local key manifest, secret-file permission, and systemd timer checks. Enabling the drain timer still needs the server-local key manifest and release approval.
...
- Mini App authentication foundation is implemented for user-facing platform API endpoints through signed Telegram `initData`. A minimal P0 Mini App page now exists for channel discovery/submission/detail with guarded pagination, channel and relay owner profile updates, relay submission, relay directory/detail with guarded pagination, model-test job creation/status with paginated job/report lists, private report viewing with structured summaries, visibility switching across private/unlisted/public modes, share-link creation, contribution entry points, and ledger viewing with account filters and pagination; full production UX is still pending.
...
- Service foundation:
  ...
  - Model Lab drain-readiness command that checks the isolated runner,
    server-local key manifest, manifest secret-file permissions, and systemd
    timer state without running drain jobs or printing raw keys,
  ...
  - Mini App Model Lab job/report list pagination with visible range/total state and disabled controls when Telegram initData is unavailable,
  - Mini App channel and relay directory pagination controls that start disabled,
    stay disabled without Telegram initData, and only enable when another page
    exists,
...
- inviter-facing reward status history with masked invited-user ids, delayed settlement status, public rejection/risk reasons, pagination, one-click appeal creation for risk/rejected rewards, and duplicate-appeal throttling while a matching appeal is open or under review,
  ...
  - ledger balance and paginated entry query helpers,
...
- `.\.venv312\Scripts\python.exe scripts\platform_ops.py model-drain-readiness-check --key-manifest-file .\model-test-keys.json --worker-runner /usr/local/libexec/tgsellbot/run-isolated-worker.sh`
  validates the isolated runner path, server-local manifest content and
  secret-file permissions, and `tgsellbot-model-test-drain.timer` state without
  running drain jobs or printing raw keys.
...
5. Model Lab P0
   - Enable the operator batch-drain timer only after the isolated Worker runner is installed, smoke-tested under `tgsellbot-worker`, and paired with a restricted server-local key manifest that passes `model-drain-readiness-check`.
...
- Local Browser smoke on `http://127.0.0.1:9393/platform/app?tab=wallet`
  loaded the Mini App wallet panel with account filters, ledger prev/next
  controls, disabled paging without Telegram initData, no `localStorage`, and
  no console errors.
